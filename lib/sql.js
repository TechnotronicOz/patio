...
let Dataset, patio, sql, Expression, AliasedExpression, CaseExpression, Cast, ColumnAll, BooleanExpression,
    ...
    NumericExpression, QualifiedIdentifier, StringExpression, SubScript, LiteralString, Json, JsonArray;
...
const DATE_METHODS = ["getDate", "getDay", "getFullYear", "getHours", "getMilliseconds", "getMinutes", "getMonth", "getSeconds",
...
const addDateMethod = function (op) {
    return function () {
 ...
 */
const Time = function (h, min, s, ms) {
 ...
 * @param {Number} [ms=0] the millisecond/s, this paramater is not be used, but may depending on the adapter.
 */
const TimeStamp = function (y, m, d, h, min, s, ms) {
    var args = argsToArray(arguments);
    if (args.length === 0) {
        this.date = new Date();
    } else if (isDate(y)) {
 ...
 * @param {Number} [ms=0] the millisecond/s, this paramater is not be used, but may depending on the adapter.
 */
const DateTime = function (y, m, d, h, min, s, ms) {
    var args = argsToArray(arguments);
    if (args.length === 0) {
        this.date = new Date();
    } else if (isDate(y)) {
...
const MATHEMATICAL_OPERATORS = {PLUS: "+", MINUS: "-", DIVIDE: "/", MULTIPLY: "*"};

// Bitwise mathematical operators used in +NumericMethods+
const BITWISE_OPERATORS = {bitWiseAnd: "&", bitWiseOr: "|", exclusiveOr: "^", leftShift: "<<", rightShift: ">>"};


const INEQUALITY_OPERATORS = {GT: ">", GTE: ">=", LT: "<", LTE: "<="};

//Hash of ruby operator symbols to SQL operators, used in +BooleanMethods+
const BOOLEAN_OPERATORS = {AND: "AND", OR: "OR"};

//Operators that use IN/NOT IN for inclusion/exclusion
const IN_OPERATORS = {IN: "IN", NOTIN: 'NOT IN'};

//Operators that use IS, used for special casing to override literal true/false values
const IS_OPERATORS = {IS: "IS", ISNOT: 'IS NOT'};

//Operator symbols that take exactly two arguments
const TWO_ARITY_OPERATORS = merge({
    EQ: '=',
    NEQ: '!=',
    LIKE: "LIKE",
    "NOT LIKE": 'NOT LIKE',
...
const N_ARITY_OPERATORS = merge({"||": "||"}, BOOLEAN_OPERATORS, MATHEMATICAL_OPERATORS);

//Operator symbols that take only a single argument
const ONE_ARITY_OPERATORS = {"NOT": "NOT", "NOOP": "NOOP"};
 ...
 * @memberOf patio.sql
 */
const AliasMethods = define(null, {
    instance: {
...
const bitWiseMethod = function (op) {
 ...
 * @memberOf patio.sql
 */
const BitWiseMethods = define(null, {
    instance: {
...
const booleanMethod = function (op) {
    return function (expression) {
 ...
 * </pre>
 */
const ComplexExpressionMethods = define({
    instance: {
        /**@ignore*/
        getters: {
...
const inequalityMethod = function (op) {
    return function (expression) {
 ...
 * @memberOf patio.sql
 */
const InequalityMethods = define({
    instance: {
 ...
 * @memberOf patio.sql
 */
const NoBooleanInputMethods = define({
    instance: {
...
const numericMethod = function (op) {
    return function (expression) {
 ...
 * @memberOf patio.sql
 */
const QualifyingMethods = define({
    instance: {
 ...
 * @memberOf patio.sql
 */
const StringConcatenationMethods = define({
    instance: {
 ...
 * @memberOf patio.sql
 */
const GenericExpression = define([Expression, AliasMethods, BooleanMethods, CastMethods, ComplexExpressionMethods, InequalityMethods, NumericMethods, OrderedMethods, StringMethods, SubscriptMethods]).as(sql, "GenericExpression");
...
const ComplexExpression = define([Expression, AliasMethods, CastMethods, OrderedMethods, SubscriptMethods], {
    instance: {
...
const Constant = GenericExpression.extend({
    instance: {
...
const JoinClause = Expression.extend({
    instance: {
...
const JoinOnClause = JoinClause.extend({
    instance: {
...
const JoinUsingClause = JoinClause.extend({
    instance: {
...
const STRING_METHODS = ["charAt", "charCodeAt", "concat", "indexOf", "lastIndexOf", "localeCompare", "match", "quote",
...
const addStringMethod = function (op) {
    return function () {
 ...
 * @memberOf patio.sql
 */
JsonArray = function JsonArray(arr) {
